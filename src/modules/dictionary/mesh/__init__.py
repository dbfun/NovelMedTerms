...
class MeSH:
    ...
    def search(self, term: str) -> Optional[TermDTO]:
        for concept in self.dict.search(term):
            ...
            # что значит термин известен системе. Возвращаем первое совпадение.
...
class DictionaryMesh(Module):
    ...
    def _mark_term_as_known(self, session: Session, dictionary_id: int, term_id: int, result: TermDTO):
